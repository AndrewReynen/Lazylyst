...
def simpleLocator(pickSet,staMeta,mapCurEve,staSort,
                  Vp=5.0,Vs=2.9,Dp=0,Ds=0,tResThresh=0.5):
    ...
    for aPick in pickSet:
        ...
        stas.append(aPick[0])
    data,stas=np.array(data),np.array(stas,dtype=str)
    ...
    poorResidStas=np.unique(stas[np.where(resids>=tResThresh)])
    goodResidStas=np.unique(stas[np.where(resids<tResThresh)])
